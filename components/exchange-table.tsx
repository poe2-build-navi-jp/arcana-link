...
export function ExchangeTable({inventory,server,status,publicId,locale='ja'}:{inventory:InventoryCounts;server:ServerRegion|'';status:ExchangeStatus;publicId?:string;locale?:SiteLocale}) {
  ...
  return <section className="exchange-table"><h3>{t.title}</h3><p>Server：{server ? serverLabels.en[server] : '—'} · {statusLabel(locale,status)}</p><dl><dt>{t.want}</dt><dd>{want||t.none}</dd><dt>{t.offer}</dt><dd>{offer||t.none}</dd></dl><a className="card-seo-action" href={`${localizedPath(locale)}${server ? '?server='+server : ''}#matches`}>{t.match}</a>
    ...
    <p>{t.private}</p><p role="status">{message}</p></section>;
}
