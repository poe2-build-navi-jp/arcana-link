...
export function ExchangeTable({inventory,server,status,locale='ja',onMatch}:{inventory:InventoryCounts;server:ServerRegion|'';status:ExchangeStatus;publicId?:string;locale?:SiteLocale;onMatch?:()=>void}) {
  ...
  return <section className="exchange-table"><h3>{t.title}</h3><p>Server：{server ? serverLabels.en[server] : '—'} · {statusLabel(locale,status)}</p><dl><dt>{t.want}</dt><dd>{want||t.none}</dd><dt>{t.offer}</dt><dd>{offer||t.none}</dd></dl><a className="card-seo-action" onClick={onMatch} href={`${localizedPath(locale)}#matches`} aria-disabled={!server}>{t.match}</a>
